...
class TestFlow(TransactionCase):
    ...
    def setUp(self):
        ...
        self.last_stage = self.env['maintenance.stage'].create({
            'name': 'Last state',
        })
        self.stage = self.env['maintenance.stage'].create({
            'name': 'New state',
            'next_stage_ids': [(4, self.last_stage.id)],
        })
    ...
    def test_form(self):
        ...
        self.assertIn(
            self.request,
            self.env['maintenance.request'].search(attr['invisible'])
        )
        getattr(self.request.with_context(
            json.loads(button_stage.attrib['context'])
        ), button.attrib['name'])()
